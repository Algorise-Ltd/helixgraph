...
- [x] Fix Delivery Date Logic: POs from 2024 should NOT have delivery dates in 2025 in `data/raw/procurement_data_generator.py`.
...
- [x] Implement `get_product_context` logic (currently placeholder).
- [x] Integrate entity extraction (NER) logic based on the code provided by Yixuan. Ask user to provide the code
- [x] Implement aggregations (total spend, counts, averages) in context retrieval.
...
- [x] Test the endpoint with `curl`.
- [x] Define Pydantic models for request/response.
- [x] Implement auto entity extraction when not provided.
- [x] Implement execution time tracking.
- [x] Implement error handling.

**Part 8.5: Test Part 7 and 8**
- [x] Reivew and test all the scripts crated for Part 7 and Part 8.
- [ ] Perform full-scale test with NER model (integration test).

**Part 9: Build ETL Pipeline for Graph Construction**
- [x] Create `etl/load_sprint2.py`.
- [x] Run the ETL script to load all data into Neo4j.
- [x] Validate the loaded graph data.

**Part 10: Document RAG Implementation**
- [x] Create `docs/rag_architecture.md`.
- [x] Include an architecture diagram.
- [x] Document example Q&A pairs.
- [x] List known limitations.
- [x] Provide a troubleshooting guide.

**Part 11: Final Review**
- [x] `sprint_plan.md` to check if there is any task not implemented.
- [x] Review all tasks in this final and check whether they are properly implemented.
- [x] Explain / show to user how each task in this document implemented, ask approval for the implementations.

**Part 12: Rework and Regenerate**
- [x] Run `python data/raw/procurement_data_generator.py` to regenerate data with fixes.
- [x] Run `python etl/load_sprint2.py` to reload the graph with new data.
- [x] Run `python etl/validate_graph.py` to validate the reloaded graph.